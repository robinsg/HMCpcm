...
class HMC(object):
    ...
    def __internal__get_preferences(self, extension):
        ...
        if self.debug:
            self.save_to_file("perferences.xml", ret.text)
        if ret.status_code != 200:
    ...
    def __internal__strip_preferences(self,pref_string,findstring):
        '''Internal function
           Uses string manipulation to find the data acceptable to the HMC during set perferences
    ...
    def parse_prefs_pcm(self,preferences):
        ...
        for this_server in self.stats_xml.findall('yyy:ManagedSystemPcmPreference',ns):
            ...
            ### Older HMC V8 versions do not support Energy Monitoring so handle the attribute error
            try:
    ...
    def set_ltm_flag(self,server,value):
        '''Set the Long Term Monitor flag
           Arguments: 1 the server name - as seen on the HMC an invalid name is ignored
           Returns: none '''
        if self.debug:
            print("DEBUG:set_ltm_flag(%s)"%(server))
        ns = {'yyy': 'http://www.ibm.com/xmlns/systems/power/firmware/pcm/mc/2012_10/'}
        for this_server in self.stats_xml.findall('yyy:ManagedSystemPcmPreference',ns):
            name    = this_server.find('yyy:SystemName',ns).text
            if name == server:
                # If ltm is false and aggregation is true then ltm will be implicitly set to true
                # Make sure both ltm and aggregation are false
                ltm     = this_server.find('yyy:LongTermMonitorEnabled',ns)
                agg     = this_server.find('yyy:AggregationEnabled',ns)
                ltm.text = value
                agg.text = value
                # break
    ...
    def get_filename_from_xml(self, xmltext):
        ...
        if self.debug:
            print("DEBUG:get_filenamefrom_xml(%s,%s)" % (fileUrl, xmltext))
    ...
    def extract_ssp_totals(self,stats):
        ...
        for sample in data["sspUtil"]["utilSamples"]:
            # Each sample is a stats snapshot for this SSP
            samplecount += 1
            ...
            if self.debug:
                print("DEBUG:%s size=%d free=%d OPs:read=%6d write=%6d Bytes:read=%9d write=%9d ServiceTime:read=%9d write=%9d" % (timeStamp,size,free,numOfReads,numOfWrites,readBytes,writeBytes,readServiceTime,writeServiceTime))
            # timeStamp[0:19] = ignore fractions of a second
            entry = {'time':timeStamp[0:19], 'size':size, 'free':free, 'numOfReads':numOfReads, 'numOfWrites':numOfWrites, 'readBytes':readBytes, 'writeBytes':writeBytes,'readServiceTime':readServiceTime, 'writeServiceTime':writeServiceTime }
            ret.append(entry)
        return ret
    ...
    def extract_ssp_vios(self,stats):
        ...
        data = json.loads(stats) # convert to JSON
        if self.debug:
            self.save_to_file("ssp_vios_string.json",data)
        viosstats = []
        samplecount = 0
        for sample in data["sspUtil"]["utilSamples"]:
            timeStamp   = sample['sampleInfo']['timeStamp']
    ...
    def tar_csv(self):
        ...
        zf = zipfile.ZipFile('pcmstats.zip', 'a')
